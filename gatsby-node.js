...
exports.createPages = async ({ graphql, actions }) => {
  ...
  const result = await graphql(`
    {
      allSanityPost(filter: { slug: { current: { ne: null } } }, sort: { order: DESC, fields: publishedAt }) {
        edges {
          node {
            title
            slug {
  ...
  const artists = result.data.allSanityAuthor.edges || []
  ...
  artists.forEach((edge, index) => {
    const path = `/artistas/${edge.node.slug.current}`
    createPage({
      path,
